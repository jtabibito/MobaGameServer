...
log::init(char* path,
	...
	string temp_path = g_log_path;
	int find = temp_path.find('/');
	uv_fs_t req;
	int result;

	while (find != std::string::npos) {
		result = uv_fs_mkdir(uv_default_loop(), &req, temp_path.substr(0, find).c_str(), 0755, NULL);
		find = temp_path.find('/', find + 1);
	}
...
open_file(tm* time_struct) {
	...
	result = uv_fs_open(uv_default_loop(), &g_file_handle, fileName, O_CREAT | O_RDWR | O_APPEND, S_IREAD | S_IWRITE, NULL);
	if (result < 0) {
...
format_time() {
	...
	if (now != g_last_second) {
		...
		sprintf(g_format_time, "%4d-%02d-%02d %02d:%02d:%02d", time_struct->tm_year + 1900, time_struct->tm_mon + 1, time_struct->tm_mday,
...
log::write2log(const char* file_name,
	...
	sprintf(msg_meta_info, "%s:%d", file_name, line_num);
	uv_buf_t buf[6];
	buf[0] = uv_buf_init(g_format_time, strlen(g_format_time));
	buf[1] = uv_buf_init(g_log_modle[modle], strlen(g_log_modle[modle]));
	...
	if (result < 0) {
		fprintf(stderr, "log failed %s%s%s%s", g_format_time, g_log_modle[modle], msg_meta_info, msg_content);
	}

	uv_fs_req_cleanup(&writeReq);

	if (g_std_output) {
		printf("%s:%u\n[%s] %s\n", file_name, line_num, g_log_modle[modle], msg_content);
	}
}